...
def calculate_transmission(a=0.95, r=0.95, n_eff=1.6, radius=50e-6, wavelength_range=(744e-9, 746e-9), num_points=40):
    """
...
def fit_transmission(wavelengths, intensity_ratios):
    ...
    popt, _ = curve_fit(transmission_model, wavelengths, intensity_ratios, p0=[0.90,0.90])
    a = popt[0]
    r = popt[1]
...
def plot_results(wavelengths, intensity_ratios, a, r, widths, internal_factors, loaded_factors, loss_values):
    ...
    plt.plot(wavelengths * 1e9, intensity_ratios, label='Data', color='b', alpha=0.5)
...
def find_minima(wavelenghts, intensity_ratios):
    ...
    peaks, _ = find_peaks(opposite_transm, distance=10, height = -0.75)
...
output_dir = r"C:\Users\Kids\Desktop\data\250315"
file = os.path.join(output_dir, "P7A18_rr100umfr.csv")

df = pd.read_csv(file)
print(df.shape)


wavelengths = df.iloc[:, 0].values * 1e-9
intens = df.iloc[:, 1].values

p_lin = 10**(intens/10)
p_norm = p_lin / np.max(p_lin)

p_norm_db = 10 * np.log10(p_norm)

intensities = 10**(p_norm_db/10)
